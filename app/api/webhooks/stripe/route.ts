...
async function handleCheckoutSessionCompleted(
  ...
  if (session.mode === 'subscription') {
    ...
    // Map plan from session metadata if available
    const planId = (session.metadata && session.metadata.planId) || 'starter';
    ...
    await supabase.from('billing_subscriptions').upsert({
      ...
      plan_id: planId,
      status: 'active',
      current_period_end: session.current_period_end ? new Date(session.current_period_end).toISOString() : null,
    });
