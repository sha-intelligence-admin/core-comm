...
-- This ensures the tables exist before we try to modify or reference them.
...
-- Create calls table (if it doesn't already exist)
...
-- Create integrations table (if it doesn't already exist)
...
-- Step 2: Add all indexes.
-- This ensures indexes are created after all tables are created.
...
CREATE TRIGGER update_company_updated_at BEFORE UPDATE ON public.company FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Step 4: Add all RLS policies.
-- These are applied after all tables and columns exist.
